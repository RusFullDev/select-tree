...
const checkboxs = ref(Array.from({ length: props.data6.length }, () => false));
const isCheck = computed({
  get() {
    return checkboxs.value.every((item) => item);
  },
  set(value) {
    checkboxs.value = Array.from({ length: props.data6.length }, () => value);
  },
});
...
const filteredData = computed(() => {
  if (!props.searchQuery) return props.data6;
  return props.data6.filter(item => item.toLowerCase().includes(props.searchQuery.toLowerCase()));
});
...
<template>
  <div class="header">
    <div class="flex gap-2 items-center mb-3">
      ...
      <div class="flex gap-1 cursor-pointer" @click="showMusic">
        <p class="font-medium text-sm">Music, Dance, Cinema</p>
