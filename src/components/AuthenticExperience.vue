...
const showAuthenticExperience = () => {
  isOpen.value = !isOpen.value;
};

const checkboxs = ref(Array.from({ length: props.data8.length }, () => false));
const isCheck = computed({
  get() {
    return checkboxs.value.every((item) => item);
  },
  set(value) {
    checkboxs.value = Array.from({ length: props.data8.length }, () => value);
  },
});
...
const filteredData = computed(() => {
  if (!props.searchQuery) return props.data8;
  return props.data8.filter(item => item.toLowerCase().includes(props.searchQuery.toLowerCase()));
});
...
<template>
  <div class="header">
    <div class="flex gap-2 items-center mb-3">
      ...
      <div class="flex gap-1 cursor-pointer" @click="showAuthenticExperience">
        <p class="font-medium text-sm">Authentic Experience</p>
