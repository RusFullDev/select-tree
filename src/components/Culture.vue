...
const checkboxs = ref(Array.from({ length: props.data3.length }, () => false));
const isCheck = computed({
  get() {
    return checkboxs.value.every((item) => item);
  },
  set(value) {
    checkboxs.value = Array.from({ length: props.data3.length }, () => value);
  },
});
...
const filteredData = computed(() => {
  if (!props.searchQuery) return props.data3;
  return props.data3.filter(item => item.toLowerCase().includes(props.searchQuery.toLowerCase()));
});
...
<template>
  <div class="header">
    <div class="flex gap-2 items-center mb-3">
      ...
      <div class="flex gap-1" @click="showCulture">
        <p class="font-medium text-sm">Culture</p>
