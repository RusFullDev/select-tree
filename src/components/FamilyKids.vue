...
const showFamilyKids = () => {
  isOpen.value = !isOpen.value;
};

const checkboxs = ref(Array.from({ length: props.data5.length }, () => false));
const isCheck = computed({
  get() {
    return checkboxs.value.every((item) => item);
  },
  set(value) {
    checkboxs.value = Array.from({ length: props.data5.length }, () => value);
  },
});
...
const filteredData = computed(() => {
  if (!props.searchQuery) return props.data5;
  return props.data5.filter(item => item.toLowerCase().includes(props.searchQuery.toLowerCase()));
});
...
<template>
  <div class="header">
    <div class="flex gap-2 items-center mb-3">
      ...
      <div class="flex gap-1 cursor-pointer" @click="showFamilyKids">
        <p class="font-medium text-sm">Family & Kids</p>
