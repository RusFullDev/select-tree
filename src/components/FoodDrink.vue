...
const checkboxs = ref(Array.from({ length: props.data.length }, () => false));
const isCheck = computed({
  get() {
    return checkboxs.value.every((item) => item);
  },
  set(value) {
    checkboxs.value = Array.from({ length: props.data.length }, () => value);
  },
});
...
const filteredData = computed(() => {
  ...
  return props.data.filter(item => item.toLowerCase().includes(props.searchQuery.toLowerCase()));
});
...
<template>
  <div class="header">
    <div class="flex gap-2 items-center mb-3">
      ...
      <div class="flex gap-1 cursor-pointer" @click="showFoodDrink">
        <p class="font-medium text-sm">Food & Drinks</p>
