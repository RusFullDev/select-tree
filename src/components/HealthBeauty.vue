...
const showHealthBeauty = () => {
  isOpen.value = !isOpen.value;
};

const checkboxs = ref(Array.from({ length: props.data4.length }, () => false));
const isCheck = computed({
  get() {
    return checkboxs.value.every((item) => item);
  },
  set(value) {
    checkboxs.value = Array.from({ length: props.data4.length }, () => value);
  },
});
...
const filteredData = computed(() => {
  if (!props.searchQuery) return props.data4;
  return props.data4.filter(item => item.toLowerCase().includes(props.searchQuery.toLowerCase()));
});
...
<template>
  <div class="header">
    <div class="flex gap-2 items-center mb-3">
      ...
      <div class="flex gap-1 cursor-pointer" @click="showHealthBeauty">
        <p class="font-medium text-sm">Health & Beauty</p>
